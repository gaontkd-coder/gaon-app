...
import MEMBERS_SEED from "./members_seed.json";
...
function MembersAdmin({ data, persist, canEdit = true, canFinance = false }) {
  // ── 출석부 회원 일괄 교체 (1회용) — 관리자·수업·팀 설정은 유지, 회원만 교체 ──
  const importMembers = async () => {
    const n = MEMBERS_SEED.length;
    const act = MEMBERS_SEED.filter((m) => m.status === "활동중").length;
    if (!window.confirm(`출석부 회원 ${n}명(활동중 ${act}명)으로 회원 명단을 전체 교체합니다.\n\n· 기존 회원은 모두 삭제됩니다.\n· 관리자(사범)·수업·팀 설정은 그대로 유지됩니다.\n\n계속할까요?`)) return;
    try {
      await persist({ ...data, members: MEMBERS_SEED });
      window.alert(`완료되었습니다. 회원 ${n}명이 입력되었습니다.\n로그인: 회원번호 + 전화번호 뒷 4자리`);
    } catch (e) {
